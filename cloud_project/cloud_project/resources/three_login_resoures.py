...
from cloud_project.resources.user_resoures import course_resource_fields
from common.models import db
from common.models.user_model import UserBase, OauthUser
from common.utils.custom_output_json import custom_output_json
from common.utils.pyjwt import _generate_token
    ...
    def get(self):
        ...
        if user:
            ...
            if user.user:
                ...
                return {'message': 'ok', 'code': 200, 'data': marshal(users, course_resource_fields), 'token': token}
...
class BindDingDing(Resource):
    ...
    def post(self):
        ...
        # db.session.commit()
        # return {'message': 'ok', 'code': 200, 'data': marshal(user, course_resource_fields), 'token': token}
        user_id = dingding.id
        db.session.add(dingding)
        db.session.commit()
        token, refresh_token = _generate_token(account, user_id)
        return {'message': 'ok', 'code': 200, 'data': marshal(user, course_resource_fields),
                'token': token}
